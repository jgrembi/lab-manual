...
\urlstyle{same}
\hypersetup{
  pdftitle={Grembi Lab Manual},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Grembi Lab Manual}
\author{}
\date{\vspace{-2.5em}Updated: 2025-10-08}
...
This manual draws heavily from the \href{https://jadebc.github.io/lab-manual/}{lab manual} of my mentor and collaborator, Dr.~Jade Benjamin-Chung. We forked that repo and several of the sections remain intact given the similar coding style that I have adopted after working with Dr.~Benjamin-Chung. Original author attributions are shown at the top of each section.
...
\chapter{Culture, conduct, and the co-lab}\label{culture-conduct-and-the-co-lab}
...
All lab members must complete \href{https://citi.psu.edu}{CITI Biomedical Human Subjects Research (IRB) Course Stage 1 - Basic Course} training and share their certificate with Jess. She will add team members to relevant Institutional Review Board protocols prior to their start date to ensure they have permission to work with identifiable datasets.
...
Team members who meet the \href{http://www.icmje.org/recommendations/browse/roles-and-responsibilities/defining-the-role-of-authors-and-contributors.html}{ICMJE Definition of authorship} will be included as co-authors on scientific manuscripts.

\section{Responsible use of AI tools}\label{responsible-use-of-ai-tools}
...
\subsection{Helpful and encouraged uses}\label{helpful-and-encouraged-uses}
...
\subsection{Use with caution: known limitations}\label{use-with-caution-known-limitations}
...
\subsection{Boundaries and responsibilities}\label{boundaries-and-responsibilities}
...
\subsection{Other contexts have other rules}\label{other-contexts-have-other-rules}
...
\section{One Health Microbiome Center Co-laboratory}\label{one-health-microbiome-center-co-laboratory}

written by Bisanz lab members
\#\#\#Background
\emph{Mission Statement:} The OHMC-CL exists to catalyze cutting edge microbiome science at PSU through facilitating independent access to cutting edge high-throughput technologies for microbiome profiling.
\emph{What the OHMC-CL is:} The OHMC-CL is collaborative space with shared equipment, resources, and protocols for microbiome characterization. The goal is for users across PSU to be able to access and use these resources to enable their research.
\emph{What the OHMC-CL is not:} The OHMC-CL is not a pay-for-service core. Typical core services are not provided. The ultimate success of any given project is the responsibility of the trainee or user. It is expected that all users will participate in scheduled training sessions and become independent users. Users who do not anticipate becoming independent users are instead encouraged to explore core services through the Huck Genomics Core or external service providers. Recommendations specific to the project can be provided upon request.
\emph{What has the OHMC-CL enabled?} In the first year and a half of operation, users have profiled 6,140 samples from a variety of sources including human, mouse, soil, plant, livestock, poultry, and environmental sources. There have been a total of 34 users, from 22 labs, from 12 different departments. Users have performed a mixture of sample extraction, qPCR, amplicon, and metagenomic sequencing.
\emph{Why does the OHMC-CL exist?}
...
\emph{Intake Form and Meeting:} All new users must complete an intake form available \href{https://docs.google.com/forms/d/e/1FAIpQLSffei_1Tbnz93hO3syqv_qpWeaec29JRwlvIQKdmADdvBZdrQ/viewform}{here} using their PSU login. The information collected in this form is used to help report usage both internally and externally. It is requested that each new project performed in the OHMC-CL has its own intake form. After reviewing the form, we will reach out to schedule a meeting with the trainee and PI to discuss project needs and to identify appropriate equipment and protocols.

\emph{Responsibilities of Trainees:}
...
\emph{Responsibilities of Supervisors:}
...
\emph{Access:} Users will be able to access the OHMC-CL during normal business hours; however, card access will not be given due to biosafety protocols.

\emph{Requesting donated reagents:} We receive an annual donation which is allocated among users. Best efforts are made to ensure allocations are evenly spread among lab groups; however, users with funded projects are encouraged to use their support to purchase consumables, thus allowing donated resources to seed future funding.

\emph{Scheduling equipment:} Equipment is available on a first-come first-served basis and must be reserved via the link in each equipment's slack channel. Frequent last minute cancellation will lead to a loss of scheduling privileges.

\emph{Contributing to the OHMC-CL:} Since the OHMC-CL runs on donated equipment, reagents, and time, it is expected that all users will give back to the OHMC-CL. Examples of how users may contribute include:
...
\emph{Protocol repository} (GitHub link){[}\url{https://github.com/BisanzLab/OHMC_Colaboratory}{]}: continually updated by users; includes validated and in-progress protocols for DNA extraction, library prep, and sequencing.

\emph{Unique Dual Indexes:} Illumina DNA prep compatible indexes based on nextera design. 574 indexes are available which are compatible with most illumina PCR-based library preps or for amplicon sequencing.

\emph{Slack Community:} user forum for troubleshooting, sharing knowledge, and training opportunities.
...
\emph{Amplicon:} Only samples which have gone through primary PCR to generate an amplicon will be included for communal sequencing runs. These may be generated using any primer set and enzyme so long as the product contains the partial Nextera overlap for indexing. A recommended protocol is available (here){[}\url{https://github.com/BisanzLab/OHMC_Colaboratory/blob/main/Protocols/AmpliconSeq/1_PrimaryPCR.md}{]}. We recommend using qPCR to generate amplicons; however, users requiring \textgreater30 cycles of PCR will need to use gel electrophoresis or other methods to ensure amplicons have formed. Users who have performed \textgreater30 cycles of PCR will be required to do a bead cleanup to remove excessive primer dimers that may cause issues downstream. Users will need to provide their samples in a skirted 96 well plate and include both a printed and digital copy of the layout using this (template){[}\url{https://github.com/BisanzLab/OHMC_Colaboratory/blob/main/Templates/0_ExtractionPlate_Template.xlsx}{]}.

\emph{Metagenomic Sequencing:} Users wishing to pool samples for metagenomic sequencing \emph{must} request indexes before preparing their libraries. Indexes must be coordinated across users on the run to prevent loss of data. Metagenomic libraries should be at least 1nM in concentration with at least 20 µL. Use of a tapestation or equivalent is required to ensure an appropriate fragment size. Libraries may be directly submitted in skirted 96 well plates including a printed and digital copy of the layout using this (template){[}\url{https://github.com/BisanzLab/OHMC_Colaboratory/blob/main/Templates/0_ExtractionPlate_Template.xlsx}{]}.

\emph{Cost Sharing for Sequencing:} Users will be required to provide an IO. This IO will be used to purchase sequencing cartridges (amplicon) or to resolve P card charges (metagenomic sequencing). Actual costs will depend on filling the sequencing run. It may take 1-2 months to resolve charges.
...
\item
  Please make an effort to respond to messages that message you (e.g., \texttt{@jess}) as quickly as possible and always within 24 hours.
\item
...
\section{Required Institutional Lab Safety Training}\label{required-institutional-lab-safety-training}

There are several required trainings before anyone can work in the lab. Please ensure you have completed all trainings \textbf{\emph{and provided certificates to the Lab Safety Officer}} before performing any work in the lab.

\subsection{\texorpdfstring{The following trainings can be accessed from the \href{https://ehs.psu.edu/training}{EHS training website}}{The following trainings can be accessed from the EHS training website}}\label{the-following-trainings-can-be-accessed-from-the-ehs-training-website}
...
\subsection{\texorpdfstring{Our lab is a Biosafety Level 2 (BSL2) lab, which requires the following trainings that can be accessed from the \href{https://citi.psu.edu/}{CITI website}}{Our lab is a Biosafety Level 2 (BSL2) lab, which requires the following trainings that can be accessed from the CITI website}}\label{our-lab-is-a-biosafety-level-2-bsl2-lab-which-requires-the-following-trainings-that-can-be-accessed-from-the-citi-website}
...
\subsection{After completing the CITI trainings, they will be populated into Penn State's Safety Tracking System (CATS).}\label{after-completing-the-citi-trainings-they-will-be-populated-into-penn-states-safety-tracking-system-cats.}
...
\item
  Directions for logging into CATS and editing your profile can be found \href{https://researchsupport.psu.edu/orp/ibc/events-training-resources/update-personnel-experience-in-cats-safety/}{here}.
\end{itemize}

\subsection{Since we work with Bloodborne Pathogens, everyone working in the lab needs to be offered updated Hepatitis B vaccination}\label{since-we-work-with-bloodborne-pathogens-everyone-working-in-the-lab-needs-to-be-offered-updated-hepatitis-b-vaccination}

\begin{itemize}
\tightlist
\item
  The Bloodborne Pathogen Immunization Determination form can be found \href{https://www.google.com/url?sa=t&source=web&rct=j&opi=89978449&url=https://ehs.psu.edu/sites/ehs/files/immunizationdetermination.doc&ved=2ahUKEwjktPLn3JuOAxVVKlkFHRmKLBgQFnoECBcQAQ&usg=AOvVaw1VFj5u8sX_YtMgqAmODOI2}{here}
\item
  This needs to be sent to Occupational Medicine, a normal course is two vaccinations one month apart
\end{itemize}

\section{Lab Safety Tour and site-specific hazard identification}\label{lab-safety-tour-and-site-specific-hazard-identification}
...
\section{Lab Safety Officer Responsibilities}\label{lab-safety-officer-responsibilities}
...
See my colleague Jade Benjamin-Chung's video on this topic: \url{https://www.youtube.com/watch?v=WoYkY9MkbRE}
...
\textbf{\emph{{[}Ziyi, add the links to the github examples here{]}}}
...
Consider using RStudio's \href{https://support.rstudio.com/hc/en-us/articles/200484568-Code-Folding-and-Sections}{code folding} feature to collapse and expand different sections of your code. Any comment line with at least four trailing dashes (-), equal signs (=), or pound signs (\#) automatically creates a code section. See the above link for more info.
...
\begin{figure}
\centering
\includegraphics{assets/images/data-masking.PNG}
\caption{Alt text}
\end{figure}
...
\chapter{Slurm and cluster computing on ROAR}\label{slurm}

by Jess Grembi

Many of the bioinformatic tools that we use require a large amount of RAM, rely on large databases, and can be run in parallel across many samples. For these types of analyses, we use ROAR Collab, Penn State's computing cluster. ROAR uses Slurm, an open source, scalable cluster management and job scheduling system for computing clusters. You will need to \href{https://www.icds.psu.edu/account-setup/}{request an account}, and once you have one Jess can add you to the lab account to access our storage areas and compute resources. Please refer to the \href{https://docs.icds.psu.edu}{ROAR User Guide} to learn about the system and how to use it. Below, we include a few tips specific to how we use ROAR Collab in our lab.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ssh \textless{}user\textgreater{}@submit.hpc.psu.edu}
\end{Highlighting}
\end{Shaded}
...
Each user account is provided with 3 different storage assets. The \texttt{\$HOME} directory is a good place to store code and small test files (quota: 16 GB per user) and the \texttt{\$WORK} directory is a good place to store larger items that you will use regularly (quota: 128 GB per user). Save intermediate processing files and temporary files generated by specific bioinformatic tools to the \texttt{\$SCRATCH} directory (quota: unlimited, but files are automatically deleted 30 days after their last modification). You can read more about storage options on ROAR \href{https://docs.icds.psu.edu/handling-data/file-storage/}{here}.

The lab has also purchased active storage space (45 TiB, accessible at \texttt{/storage/group/jag548/default}) and archive storage (15 TiB, accessible at \texttt{/storage/archive/jag548/default}). Archive storage is about 1/4 the cost of active storage so raw sequencing files should be moved to the archive storage once you've completed the preliminary data processing steps and are fairly certain you will not be needing them again anytime soon.

You can check the use of your personal storage and the group resources easily by running:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{check\_quota}
\end{Highlighting}
\end{Shaded}
...
Your credit account '' jag548\_cr\_credit'' is active. To use the account, you will need to specify it in your job submission by either selecting it via the drop down options on the portal, or by using the directive ``--account= jag548\_cr\_credit''. Additionally, you will also need to specify the desired hardware partition for your job to launch correctly. The options are ``basic'', ``standard'', and ``himem''.
...
Open the Makevars file in ROAR Collab (\texttt{vi\ \textasciitilde{}/.R/Makevars}) and append the following lines
...
\section{Moving files to ROAR Collab}\label{moving-files-to-roar-collab}
...
\CommentTok{\# (not after ssh{-}ing into ROAR Collab)}

\CommentTok{\# securely transfer folders from Box to ROAR Collab work directory}
\CommentTok{\# note: the {-}r option is for folders and is not needed for files}
\FunctionTok{scp} \AttributeTok{{-}r} \StringTok{"Box/malaria{-}project/folder{-}1/"}\NormalTok{ USERNAME@submit.hpc.psu.edu:/work/}

\CommentTok{\# securely transfer folders from Box to your ROAR Collab scratch directory}
\FunctionTok{scp} \AttributeTok{{-}r} \StringTok{"Box/malaria{-}project/folder{-}2/"}\NormalTok{ USERNAME@submit.hpc.psu.edu:/scratch/}

\CommentTok{\# securely transfer folders from Box to our shared storage directory}
\FunctionTok{scp} \AttributeTok{{-}r} \StringTok{"Box/malaria{-}project/folder{-}3/"}\NormalTok{ USERNAME@submit.hpc.psu.edu:/storage/group/jag548/}
\end{Highlighting}
\end{Shaded}
...
\subsection{The ROAR Collab OnDemand Dashboard}\label{the-roar-collab-ondemand-dashboard}

The second way to test and edit code is to use the ROAR Collab OnDemand Dashboard, accessed by typing \url{login.sherlock.stanford.edu} into a web browser. You will be prompted to authenticate the way you would for any Stanford website. This is the best way to edit code for people who are not comfortable accessing \& editing in base R in a Shell application.
...
\subsection{Filepaths \& configuration on ROAR Collab}\label{filepaths-configuration-on-roar-collab}
...
\tightlist
\item
  \href{https://monashdatafluency.github.io/r-intro-2/}{Introduction to R}
\item
  \href{https://www.kdnuggets.com/2016/03/datacamp-r-learning-path-7-steps.html}{R Learning Path: from beginner to expert in 7 steps}
\item
  \href{https://swirlstats.com/students.html}{Swirl}
\item
  \href{https://biox-rbootcamp.github.io/material}{Stanford R Bootcamp - Summer 2020}
\item
  \href{http://adv-r.had.co.nz/Style.html}{R Style Guide}
\end{itemize}

\subsection{For Intermediate/Advanced Users}\label{for-intermediateadvanced-users}
...
\section{Resources for Git \& Github Specifically tailored for R users}\label{resources-for-git-github-specifically-tailored-for-r-users}
...
\item
  \href{https://jcoliver.github.io/learn-r/010-github.html}{Git and Github in RStudio}
\item
  \href{https://gist.github.com/Z3tt/3dab3535007acf108391649766409421}{Setting your GitHub credentials}
\end{itemize}
