...
\author{}
\date{\vspace{-2.5em}Updated: 2026-06-05}
...
\textbf{\emph{The lab Safety Officers are Denise Coslo and Pablo Ochoa-Andersen}}
...
\section{Getting added to our Lab's Biosafety Protocol under Penn State's Safety Tracking System (CATS).}\label{getting-added-to-our-labs-biosafety-protocol-under-penn-states-safety-tracking-system-cats.}

\begin{itemize}
\tightlist
\item
  You will need to add information on your experience working in a laboratory.
\item
  For example, if you have prior experience working with BSL-2 organisms, that should be noted.
\item
  If you're here to gain lab experience, you can include lab-based classes you have taken (e.g.~chemistry, biology, microbiology) and also include the name of your mentor in the lab who will train you.
\item
...
\section{Since we work with Bloodborne Pathogens, everyone working in the lab is offered updated Hepatitis B vaccination, free of charge}\label{since-we-work-with-bloodborne-pathogens-everyone-working-in-the-lab-is-offered-updated-hepatitis-b-vaccination-free-of-charge}
...
\item
  Fill this out and send it to \href{https://hr.psu.edu/current-employee/occupational-medicine}{Occupational Medicine}.
\item
  They will review your immunization record and contact you with their recommendation for a booster or new vaccination series, if needed. If you have not been contacted within 3 weeks after submitting the form, please follow up with Occupational Health directly.
\item
  A normal course for an unvaccinated person is two shots one month apart, a booster would be just one shot. Student vaccinations are given at the Student Health Center on campus, staff and employees get theirs at Occupational Health (near the hospital).
\end{itemize}
...
\section{Use Zotero for Citing Sources}\label{use-zotero-for-citing-sources}
...
\tightlist
\item
  First, download the desktop version of Zotero. Using the following link: \href{https://www.zotero.org/download/}{Zotero Download Page}
\item
...
\item
  If you accidently dismiss the pop-up or it does not appear, follow the installation instructions here: \href{https://www.zotero.org/support/word_processor_plugin_installation}{Word Plugin Installation Guide}
\item
  To create a Zotero account, use the registration page below: \href{https://www.zotero.org/user/register/}{Zotero Account Registration}
...
\item
  For additional help getting started, refer to Zotero's Quick Start Guide: \href{https://www.zotero.org/support/quick_start_guide}{Zotero Quick Start Guide}
\item
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{assets/images/data-masking.PNG}}
\caption{Alt text}
\end{figure}
...
\section{\texorpdfstring{Monitoring Job Efficiency and Resource Usage (\texttt{seff})}{Monitoring Job Efficiency and Resource Usage (seff)}}\label{monitoring-job-efficiency-and-resource-usage-seff}

When you submit a job on ROAR Collab, you request specific resources (CPUs, memory, and time). However, \textbf{your allocation only lasts while the job runs} -- not for the entire duration you requested.
For example: If you request \textbf{48 hours}, but your program finishes in \textbf{2 hours}, the scheduler automatically frees those CPUs and memory after 2 hours.
You don't ``hold the node'' for the remaining 46 hours.
The real waste occurs only when you \textbf{over-request CPUs or memory} that your code doesn't actually use.
Those resources remain idle, which shows up as low efficiency when you check your job with \texttt{seff}.

\textbf{Checking job efficiency with \texttt{seff}}

After a job finishes, run:

\texttt{seff\ \textless{}JOBID\textgreater{}}

Example output:

Job ID: 1234567
Cluster: roar-collab
User/Account: nbt5288/jag548\_cr\_credit
...
Interpretation:
- \textbf{CPU Efficiency:} (CPU time used / CPU time allocated) × 100
- If \textless50\%, you requested too many CPUs.
- \textbf{Memory Efficiency:} (Memory used / Memory allocated) × 100
- If \textless50\%, you overestimated memory.
A high efficiency (\textgreater80\%) means your resource requests are well tuned.

\textbf{Recommended workflow for tuning jobs}
1. Start small.
...
\texttt{sacct\ -j\ \$SLURM\_JOB\_ID\ -\/-format=JobID,JobName,Elapsed,TotalCPU,AllocCPUS,MaxRSS,AllocTRES,State}
...
For more details, see the ROAR documentation on \href{https://docs.icds.psu.edu/running-jobs/batch-jobs/\#slurm-commands}{batch jobs and Slurm commands}.

Why it's important to monitor job efficiency and usage

\textbf{Fairshare} is a slurm algorithm that influences a job's priority based on a user's recent consumption of cluster resources compared to their allocated share. \emph{To maintain a higher Fairshare score and ensure your jobs have priority, avoid requesting more resources than necessary}. This not only optimizes your job efficiency but also contributes to a fairer resource distribution among users.

Here's a breakdown of how it works:
- \textbf{Balancing access:} The Fairshare system ensures that no single user or group can monopolize the cluster. If you have recently used a large portion of the resources, the priority of your pending jobs is lowered. If you have used fewer resources, your jobs get a boost in priority.
- \textbf{Resource tracking:} Fairshare tracks your usage based on the resources you request, such as CPU cores, memory, and GPUs, multiplied by the job's runtime. This means requesting more resources than you need can significantly reduce your Fairshare score.
- \textbf{Priority components:} The fairshare factor is a component of a job's overall priority score, which also includes other elements like:
- \textbf{Age:} The longer a job has been waiting in the queue, the higher its priority becomes.
- \textbf{Job size:} The size of the job (how many resources it requests) can also influence its priority. Properly monitoring your computing efficiency can help you minimize a job's size and raise the likelihood that there will be sufficient free resources to execute your job before other, larger, jobs in the queue.
- \textbf{Quality of Service (QoS):} This is a policy tag that can give jobs a priority boost.
- \textbf{Nice factor:} A value that users can set to manually lower the priority of their own jobs.
...
\section{Grembi Lab Compute Resources and Verification}\label{grembi-lab-compute-resources-and-verification}

Dr.~Jessica Grembi's lab has \textbf{paid ROAR Collab compute resources} under the account name: jag548\_cr\_credit
This account allows lab members to submit jobs to ROAR's \textbf{credit-based compute partitions} (\texttt{basic}, \texttt{standard}, and \texttt{himem}) and charge usage to the lab's funded credits.

\textbf{How to check if you have access}
1. Check your accounts with \texttt{get\_balance} or \texttt{mybalance}
- You should see jag548\_cr\_credit listed if it's active for you.

For example, this is what Nazifa sees:
Account Type Resources\\
-------- ------- ----------------\\
jag548\_cr\_credit Credit 100.0000 credits\\
one\_sc\_default Allocation cpu=120,gres/gpu=0,mem=960G\\
...
\tightlist
\item
  If jag548\_cr\_credit is missing, contact Dr.~Grembi and ask to be added to the credit account. Once added, you'll be able to submit jobs using:
\end{enumerate}
